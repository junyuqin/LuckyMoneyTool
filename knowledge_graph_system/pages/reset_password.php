...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'send_code') {
        ...
        if (empty($phone)) {
            ...
            // 发送验证码
            $code = sendVerificationCode($phone, 'reset_password');
        ...
        if (empty($phone)) {
            ...
            $result = verifyCode($phone, $code, 'reset_password');
            if ($result['valid']) {
        ...
        // 验证
        if (empty($phone) || !($_SESSION['reset_verified'] ?? false)) {
            $errors[] = "请先完成手机验证";
            $step = 1;
            ...
            if (empty($errors)) {
                ...
                if ($stmt->execute() && $db->changes() > 0) {
                    $successMessage = "密码重置成功！即将跳转到登录页面...";
...
// 获取当前步骤的手机号
$currentPhone = $_SESSION['reset_phone'] ?? '';
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
